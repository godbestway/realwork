...
int local_action_put_allflows(ActionAllState *recv_state)
{
    ...
    printf("local_action_put_allflows finished \n");
    return 1;
}

/*
int local_get_allflows()
{
	int result = conn_send_perflow(uint8_t* buf, int len)

	ActionState* action_perflow = (ActionState*)malloc(sizeof(ActionState));;
	action_state__init(action_perflow);

	PradsStat *send_pr_s = (PradsStat*)malloc(sizeof(PradsStat));

	send_pr_s->has_got_packets=1; 
        send_pr_s->got_packets = config.pr_s.got_packets;
	send_pr_s->has_eth_recv=1; 
        send_pr_s->eth_recv = config.pr_s.eth_recv;
	send_pr_s->has_arp_recv=1; 
        send_pr_s->arp_recv = config.pr_s.arp_recv;
	send_pr_s->has_otherl_recv=1; 
        send_pr_s->otherl_recv = config.pr_s.otherl_recv;
	send_pr_s->has_vlan_recv=1; 
        send_pr_s->vlan_recv = config.pr_s.vlan_recv;
	send_pr_s->has_ip4_recv=1; 
        send_pr_s->ip4_recv = config.pr_s.ip4_recv;
	send_pr_s->has_ip6_recv=1; 
        send_pr_s->ip6_recv = config.pr_s.ip6_recv;
	send_pr_s->has_ip4ip_recv=1; 
        send_pr_s->ip4ip_recv = config.pr_s.ip4ip_recv;
	send_pr_s->has_ip6ip_recv=1; 
        send_pr_s->ip6ip_recv = config.pr_s.ip6ip_recv;
	send_pr_s->has_gre_recv=1; 
        send_pr_s->gre_recv = config.pr_s.gre_recv;
	send_pr_s->has_tcp_recv=1; 
        send_pr_s->tcp_recv = config.pr_s.tcp_recv;
	send_pr_s->has_udp_recv=1; 
        send_pr_s->udp_recv = config.pr_s.udp_recv;
	send_pr_s->has_icmp_recv=1; 
        send_pr_s->icmp_recv = config.pr_s.icmp_recv;
	send_pr_s->has_othert_recv=1; 
        send_pr_s->othert_recv = config.pr_s.othert_recv;
	send_pr_s->has_assets=1; 
        send_pr_s->assets = config.pr_s.assets;
	send_pr_s->has_tcp_os_assets=1; 
        send_pr_s->tcp_os_assets = config.pr_s.tcp_os_assets;
	send_pr_s->has_udp_os_assets=1; 
        send_pr_s->udp_os_assets = config.pr_s.udp_os_assets;
	send_pr_s->has_icmp_os_assets=1; 
        send_pr_s->icmp_os_assets = config.pr_s.icmp_os_assets;
	send_pr_s->has_dhcp_os_assets=1; 
        send_pr_s->dhcp_os_assets = config.pr_s.dhcp_os_assets;
	send_pr_s->has_tcp_services=1; 
        send_pr_s->tcp_services = config.pr_s.tcp_services;
	send_pr_s->has_tcp_clients=1; 
        send_pr_s->tcp_clients = config.pr_s.tcp_clients;
	send_pr_s->has_udp_services=1; 
        send_pr_s->udp_services = config.pr_s.udp_services;
	send_pr_s->has_udp_clients=1; 
        send_pr_s->udp_clients = config.pr_s.udp_clients;

	action_perflow->all_state = send_pr_s;
	MyActionMessage mes;
        my_action_message__init(&mes);	
...
int local_put_allflows(char *state)
{
    // Decode allflows state
