...
int conn_send_getPerflowAck(int count){
	...
	int result;
	result = send_proto_object(connac_conn_state,getPerflowAck_object);
	return result;

}

int conn_send_putPerflowAck(int count){
	ProtoObject * perflowack_object;
	perflowack_object = proto_compose_putPerflowack_message(count);
	int result;
	result = send_proto_object(connac_conn_state, perflowack_object);
	return result;
}
...
static int handle_put_perflow(PutPerflowMsg* putPerflow_recv)
{
    ...
    conn_send_putPerflowAck(putPerflow_recv->count);
