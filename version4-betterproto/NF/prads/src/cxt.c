...
int local_action_put_perflow(ActionState* recv_state){
	...
	put_asset_per_sip(recv_state->s_ip, action_state->c_asset); 
    }
   if(action_state->s_asset != NULL){
	put_asset_per_sip(recv_state->d_ip, action_state->s_asset); 
    }
...
int local_action_get_one_perflow(Match *match){
	...
	int hash = match->hash;
 	int cxid = match->cxid;
	printf("local get one action per flow ---finished read match\n");
	...
	while(action_state != NULL){
		...
		Asset* c_asset = serialize_c_asset(action_state);
		if(c_asset != NULL){
			action_perflow->c_asset = c_asset;
			action_perflow->has_s_ip= 1;
			action_perflow->s_ip = match->nw_src;
		}

		Asset* s_asset = serialize_s_asset(action_state);
		if(s_asset != NULL){
			action_perflow->s_asset = s_asset;
			action_perflow->has_d_ip= 1;
			action_perflow->d_ip = match->nw_dst;
		}
...
int local_conn_get_perflow(Key key){
    ...
    {        
		...
		if ((!(key.wildcards & WILDCARD_NW_PROTO)) &&((conn_state->proto) != key.nw_proto))
	     ...
	     match->nw_src = conn_state->s_ip.s6_addr32[0];
	     match->nw_dst = conn_state->d_ip.s6_addr32[0];
