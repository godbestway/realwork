...
connState *conn_put_bucket[BUCKET_SIZE];
actionState *action_put_bucket[BUCKET_SIZE];
...
int local_conn_put_perflow(ConnState* recv_state){
    ...
    recv_state->hw_proto = recv_state->hw_proto;
    recv_state->proto = recv_state->proto;
    connState* head = conn_put_bucket[hash];

    // Add to linked list
    conn_state->prev = NULL;
    if (head != NULL)
    {
	printf("receive conn state, begin to putPerFlow ---------head lives\n");
        head->prev = conn_state;
        conn_state->next = head;
    }
    else
    { 
	printf("receive conn state, begin to putPerFlow ---------head NULL\n");
	conn_state->next = NULL; 
    }
    conn_put_bucket[hash] = conn_state;
    printf("receive conn state, begin to putPerFlow ---------before show\n");
    showPutConnState();
...
int local_action_put_perflow(ActionState* recv_state){
    ...
    printf("receive action state, begin to putPerFlow------start");
    ...
    action_state->check = recv_state->check;
     ...
     printf("receive action state, begin to putPerFlow------middle");
    uint32_t hash = recv_state->hash;
    action_state->hash = hash;

    actionState* head = action_put_bucket[hash];
    ...
    action_put_bucket[hash] = action_state;
    printf("receive action state, begin to putPerFlow------finished");
    showPutActionState();
...
void showPutConnState(){
 ...
 for (h = 0; h < BUCKET_SIZE; h++)
    {        
        connState *conn_state = conn_put_bucket[h];
...
void showPutActionState(){
 ...
 for (h = 0; h < BUCKET_SIZE; h++)
    {        
        actionState *action_state = action_put_bucket[h];
...
int local_action_get_one_perflow(int hash, int cxid){
	...
	while(action_state != NULL){
		...
		action_state__init(action_perflow);
        	...
        	action_perflow->has_start_time=1; 
...
static void *action_sender(void *arg){
	...
	//printf("match hash %d\n", hash);
	//printf("match cxid %d\n", cxid);
	int send_action = local_action_get_one_perflow(hash,cxid);
	if(send_action < 0){
		printf("send failed");
	}
	free(match);
