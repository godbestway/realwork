...
int local_conn_get_one_perflow(ConnState* connstate);
...
int local_conn_get_perflow(Key key){
	...
	for(i = 0; i< LENGTH; i++){
		...
		while(conn_head[i] != NULL){
			...
			ConnState* connstate = (ConnState*)malloc(sizeof(ConnState));
			conn_state__init(connstate);
			...
			connstate->cxid = cxid;
			int err;
			if((err = pthread_create(&conn_thread, NULL, conn_sender, (void*)connstate))!=0)
               		{
                        perror("pthread_create error");
                	}
...
static void *conn_sender(void *arg){
	...
	ConnState* connstate = (ConnState*)arg;
	int send_conn = local_conn_get_one_perflow(connstate);
	if(send_conn < 0){
		printf("send failed");
	}
	free(connstate);
