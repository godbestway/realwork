...
static void action_writeRawVarint32(int conn, int value);
...
int send_action_syn_message(int conn){
	...
	syn.has_pid = 1;
        ...
        mes.data_type = MY_ACTION_MESSAGE__DATA_TYPE__SynType;
        mes.message_case = MY_ACTION_MESSAGE__MESSAGE_ACTIONSYN;
        mes.actionsyn = &syn;
    	...
    	int result = send_action_proto_object(conn,buf, len );
	if(result < 0){
		printf("send failed");
		return -1;
	}
...
int send_action_proto_object(int conn, uint8_t* buf, int len ){
	...
	action_writeRawVarint32(conn, len);
        

	int result = action_write(conn, buf, len);
	if(result < 0){
		return -1;
	}
...
static void action_writeRawVarint32(int conn, int value) {
	...
	int result;
	    ...
	    uint8_t out_while = (uint8_t)out;
            ...
            result = action_write(conn, &out_while, 1);
	    if(result < 0){
		printf("write wrong1\n");
		}
            value = value >>7;
        ...
        uint8_t out_last = (uint8_t)value;
	result = action_write(conn, &out_last, 1);
	if(result < 0){
		printf("write wrong2\n");
	}
}
