...
//+++set cache state
    ...
    cache_state->state = action_state->state;
...
rule_type_t process_with_action_state(packetinfo *pi){
	...
	if(0 == find_action){
		printf("packet not match cache, not find the action");
		...
		printf("packet not match cache,find the action");
	} 
