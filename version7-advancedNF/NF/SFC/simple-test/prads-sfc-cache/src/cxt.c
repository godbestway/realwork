...
/*  
    ...
    conn_state->ether_dst[0] = pi->eth_hdr->ether_dst[0];        //
    conn_state->ether_dst[1] = pi->eth_hdr->ether_dst[1];        
    conn_state->ether_dst[2] = pi->eth_hdr->ether_dst[2];       
    conn_state->ether_dst[3] = pi->eth_hdr->ether_dst[3];        
    conn_state->ether_dst[4] = pi->eth_hdr->ether_dst[4];       
    conn_state->ether_dst[5] = pi->eth_hdr->ether_dst[5];       
    
    //uint8_t *ether_src = (uint8_t*)malloc(6);
    
    conn_state->ether_src[0] = pi->eth_hdr->ether_src[0];        // src MAC 
    conn_state->ether_src[1] = pi->eth_hdr->ether_src[1];        
    conn_state->ether_src[2] = pi->eth_hdr->ether_src[2];       
    conn_state->ether_src[3] = pi->eth_hdr->ether_src[3];        
    conn_state->ether_src[4] = pi->eth_hdr->ether_src[4];        
    conn_state->ether_src[5] = pi->eth_hdr->ether_src[5];        

*/
    //showConnState(conn_state);
    //printf("conn_state->hash %u",conn_state->hash);
    //printf("conn_state->cxid %u",conn_state->cxid);
//+++sfc support+++
    //local_conn_get_one_perflow(conn_state);
    //new_packet = 1;
    //wait = 1;
//+++sfc support+++
...
//+++add cache 
    ...
    //printf("pi->cxid %u\n",pi->cxid);
    if(cache_state->cxid == pi->cxid){
	//printf("match the cache state\n");
        return action_update_client(cache_state->action_state, pi);
    }
//+++add cache     
	...
	//hash = CXT_HASH4(IP4ADDR(ip_src),IP4ADDR(ip_dst),src_port,dst_port,pi->proto);
    	hash = CXT_HASH4(pi->ip4->ip_src,pi->ip4->ip_dst,pi->s_port,pi->d_port,pi->proto);
	pi->hash = hash;
	}   
    ...
    while (action_state != NULL) {
	    ...
	    if (pi->cxid == cxid){
                ...
                // Client sends first packet (TCP/SYN - UDP?) hence this is a client
		cache_state->cxid = cxid;
		cache_state->action_state=action_state;
