...
//+++
	...
	if(pi->ip4 != NULL && pi->ip4->ip_tos == 0){
...
*/
    if(config.writeiface){
    	bytes_sent=write(sock, packet ,pheader->caplen);
    	//printf("Sent %d bytes\n",bytes_sent);
    	if (bytes_sent < 0) {
            perror("sendto");
            exit(1);
   ...
   if(config.pr_s.tcp_recv == 10000){
	...
	printf("STATS: PERFLOW: end_serialize.tv_sec = %lds\n", end_serialize.tv_sec);	
    	printf("STATS: PERFLOW: end_serialize.tv_usec = %ldus\n", end_serialize.tv_usec);
	game_over();
    }
...
void prepare_tcp (packetinfo *pi)
{
	...
	conn_tracking(pi);
        ...
        uint32_t cxid = (uint16_t)((pi->ip4->ip_ttl | 0x00 ) <<8) | (uint16_t)(pi->ip4->ip_tos  | 0x00 );
	pi->cxid = cxid;
    }
...
//    }
    ...
    { 
    	...
    	sock = socket( PF_PACKET , SOCK_RAW , IPPROTO_RAW) ;            //For sending
    	memset(&daddr, 0, sizeof(struct sockaddr_ll));
    	daddr.sll_family = AF_PACKET;
    	daddr.sll_protocol = htons(ETH_P_ALL);
    	daddr.sll_ifindex = if_nametoindex(config.writeiface);
    	...
    	snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), config.writeiface);
...
//+++ verify the order of sfc
    if (config.order && (atoi(config.order) == 1))
    { 
        printf("this is the first place\n");
	first_place = 1; 
    }
