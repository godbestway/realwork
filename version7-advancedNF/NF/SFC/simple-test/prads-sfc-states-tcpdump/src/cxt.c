...
extern struct timeval start_serialize, end_serialize;
...
//+++advanced part, differnt from normal process
    int find = 0;
    actionState* find_head = action_bucket[hash];
    while(find_head != NULL){
	printf("find_head != NULL %u\ņ",find_head->cxid);
	if(find_head->cxid == cxid){
		find = 1;
		printf("action state is already there\n");	
	}
	find_head = find_head->next;
    }

    if(find == 0){
	printf("action state need to be created\n");
...
/*
    ...
    connPutPerflowAckMsg.has_pid = 1;
    connPutPerflowAckMsg.pid=pid;
    connPutPerflowAckMsg.host = (char*)malloc(CONTENT_LEN);
    connPutPerflowAckMsg.host=host;
...
void showMostPkts(){
	...
	for (h = 0; h < BUCKET_SIZE; h++)
		...
		while (action_state != NULL)
        	{     
		    if(pkts_num < action_state->s_total_pkts){
			pkts_num = action_state->s_total_pkts;
