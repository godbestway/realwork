...
int local_conn_put_perflow(ConnState* recv_state){
    ...
    uint32_t external_ip = recv_state->d_ip;
    conn_state-> external_ip = external_ip;
    ...
    connState* conn_head = conn_bucket[hash];

    // Add to linked list
    conn_state->prev = NULL;
    if (conn_head != NULL)
    {
        conn_head->prev = conn_state;
        conn_state->next = conn_head;
    }
    else
    { 
...
//+++sfc support+++
    ...
    action_state->external_ip = external_ip;
    ...
    connPutPerflowAckMsg.has_nat_hash = 1;
    connPutPerflowAckMsg.nat_hash = nat_hash;
