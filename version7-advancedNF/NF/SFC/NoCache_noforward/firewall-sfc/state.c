...
rule_type_t process_with_action_state(packetinfo *pi){
	...
	/*
	struct timeval find_before,find_after,find_middle,find_after_before;
	gettimeofday(&find_before, NULL);
        long sec = find_before.tv_sec - start_serialize.tv_sec;
        long usec = find_before.tv_usec - start_serialize.tv_usec;
        long total = (sec * 1000 * 1000) + usec;
        printf("STATS: PERFLOW: find before TIME TO process packet = %ldus\n", total);
	*/
	...
	/*gettimeofday(&find_middle, NULL);
        sec = find_middle.tv_sec - start_serialize.tv_sec;
       	usec = find_middle.tv_usec - start_serialize.tv_usec;
	...
	*/	
	...
	//cache_state->state = s;   
	/*gettimeofday(&find_after, NULL);
        sec = find_after.tv_sec - start_serialize.tv_sec;
       	usec = find_after.tv_usec - start_serialize.tv_usec;
        total = (sec * 1000 * 1000) + usec;		
        printf("STATS: PERFLOW: find after TIME TO process packet = %ldus\n", total);
	*/
	find_action = 0;
        if(s==OPEN){
        	return PASS;
