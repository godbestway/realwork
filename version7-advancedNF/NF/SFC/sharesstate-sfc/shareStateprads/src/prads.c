                ...
                const u_char * packet)
{
    ...
    if(config.pr_s.ip4_recv == 1){
    	gettimeofday(&start_serialize, NULL);
    }

    if(config.live == NULL && config.pr_s.got_packets == 20000){
	...
	printf("start time sec: %lu",start_serialize.tv_sec);
	printf("start time usec %lu",start_serialize.tv_usec);
	printf("end time sec: %lu",end_serialize.tv_sec);
	printf("end time usec %lu",end_serialize.tv_usec);
	printf("total time: %lu",total);
	game_over();
    }
    ...
    if (pi->eth_type == ETHERNET_TYPE_IP) {
        prepare_ip4(pi);
        parse_ip4(pi);
	uint16_t temp = (pi->ip4->ip_src) & 0x0000FFFF;
        //printf("temp 16: %x\n",temp);
        if(!(temp == 0xA8C0)){
...
void showPacket(packetinfo *pi){
   ...
   int i;  
   for(i = 0; i < 6;i++){
	printf("%x:",pi->eth_hdr->ether_src[i]);
 	}
   printf("\n");
   printf("dst_mac : ");   
   for(i = 0; i < 6;i++){
	printf("%x:",pi->eth_hdr->ether_dst[i]);
 	}
   printf("\n");
   ...
   // now get it back and print it
    inet_ntop(AF_INET, &(pi->ip4->ip_src), src_str, INET_ADDRSTRLEN);
     printf("src_ip %s\n", src_str);   
   ...
   // now get it back and print it
    inet_ntop(AF_INET, &(pi->ip4->ip_dst), dst_str, INET_ADDRSTRLEN);
   
   
    printf("ds_ip %s\n", dst_str);
    printf("src_port %u\n", htons(pi->s_port));
    printf("dst_port %u\n", htons(pi->d_port));
    printf("server or client %u\n",pi->sc);
}
