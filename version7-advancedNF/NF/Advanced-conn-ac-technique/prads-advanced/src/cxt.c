...
int local_action_put_perflow(ActionState* recv_state){
    ...
    time_t start_time = recv_state->start_time;
    time_t last_pkt_time = recv_state->last_pkt_time;
    uint32_t cxid = recv_state->cxid;
    uint8_t  reversed = recv_state->reversed;
    uint32_t af = recv_state->af;
    uint64_t s_total_pkts = recv_state->s_total_pkts;
    uint64_t s_total_bytes= recv_state->s_total_bytes;
    uint64_t d_total_pkts = recv_state->d_total_pkts;
    uint64_t d_total_bytes = recv_state->d_total_bytes;
    uint8_t s_tcpflags = recv_state->s_tcpflags;
    uint8_t __pad__ = recv_state->pad;
    uint8_t d_tcpflags = recv_state->d_tcpflags;  
    uint8_t check = recv_state->check;
    uint32_t hash = recv_state->hash;
...
//+++advanced part, differnt from normal process
    ...
    /*if(target_state == NULL){
        ...
        action_state->start_time = start_time;
        action_state->last_pkt_time = last_pkt_time;
        action_state->cxid = cxid;
        action_state->reversed = reversed;
        action_state->af = af;
        action_state->s_total_pkts = s_total_pkts;
        action_state->s_total_bytes= s_total_bytes;
        action_state->d_total_pkts = d_total_pkts;
        action_state->d_total_bytes = d_total_bytes;
        action_state->s_tcpFlags = s_tcpflags;
        action_state->__pad__ =__pad__;
        action_state->d_tcpFlags =d_tcpflags;  
        action_state->check = check;
        action_state->hash = hash;
	...
	if(target_state->last_pkt_time < last_pkt_time){
        ...
        target_state->reversed = reversed;
        target_state->af = af;
        target_state->s_total_pkts = target_state->s_total_pkts + s_total_pkts;
        target_state->s_total_bytes= target_state->s_total_bytes + s_total_bytes;
        target_state->d_total_pkts = target_state->d_total_pkts + d_total_pkts;
        target_state->d_total_bytes = target_state->d_total_bytes + d_total_bytes;
        target_state->s_tcpFlags = s_tcpflags;
