...
//+++
    ...
    if (from_interface_id == INTERFACE_INTERNAL) {
   	uint16_t temp = (ip->saddr) & 0x0000FFFF;
        //printf("temp 16: %x\n",temp);
        if(!(temp == 0xA8C0)){
		//printf("don't process the packet, which is original from outside\n");
		return 0;
	}
	...
	uint32_t nat_hash;
	//TODO...this number need to be recalculated after migration, because ip->saddr(s) in srcNF, dstNF are different 
	//1. add the full 5-tuple in conn_state (dst_ip, dst_port )2. after migration use local ip, external port, dst_ip, dst_port calculate the nat_hash
	//3. store nat_hash in action_state,if not exist, then create one action_state->nat_hash = nat_hash
	//4.migrate action_state, don't give the nat_hash, don't need to transfer it
