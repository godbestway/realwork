...
//new ad conn-ac 
//using for left conn_states
actionState *send_action_bucket[BUCKET_SIZE];
connState *left_conn_bucket[BUCKET_SIZE];
int left_conn = 0;
int send_action;
static pthread_t action_second_thread;
...
int local_action_thread_perflow(actionState* action_state){
...
static void *action_second_sender(void *arg){
	actionState* action_state = (actionState*)arg;

	int send_action = local_action_thread_perflow(action_state);
	if(send_action < 0){
		printf("send failed");
	}
...
int local_conn_get_perflow(Key key){
    ...
    {        
		...
		send_action_bucket[count] = action_state;
		break;
...
int local_action_get_perflow(Key key){
	...
	pthread_mutex_lock(&ActionEntryLock);
	int count = 0;
	for (h = 0; h < left_conn; h++)
        { 
		connState *conn_state = left_conn_bucket[h];
	...
	for (h = 0; h < send_action; h++)
        {
		actionState* action_state = send_action_bucket[h];
		//int err;
		//if((err = pthread_create(&action_second_thread, NULL, action_second_sender, (void*)action_state))!=0)
             	//{
                // 	perror("pthread_create error");
             	//}
	...
	action_send_getPerflowAck(count+send_action);    
...
//+++
    ...
    conn_state->proto = pi->h_ip->proto;
    ...
    if(get_conn == 1){
	printf("get_conn = %d\n",get_conn);
    	left_conn_bucket[left_conn] = conn_state;
    	left_conn++;
    }
